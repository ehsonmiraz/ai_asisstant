...
class STTEngine:
    ...
    def __init__(self):
        cm.console_output(info_log="Configuring Mic..")
        self.recognizer = sr.Recognizer()
        self.recognizer.pause_threshold = 0.5
        self.microphone = sr.Microphone()
        cm.console_output(info_log="Microphone configured successfully!")
    ...
    def recognize_input(self, already_activated=False):
        ...
        while True:
            ...
            cm.console_output("transcript: "+transcript)
    ...
    def _recognize_speech_from_mic(self, ):
        ...
        try:
            ...
            cm.console_output(info_log='User said: {0}'.format(transcript))

        except sr.UnknownValueError:
            # speech was unintelligible
            transcript = ''
            cm.console_output(info_log='Unable to recognize speech', refresh_console=False)
        except sr.RequestError:
            # API was unreachable or unresponsive
            transcript = ''
            cm.console_output(error_log='Google API was unreachable')
...
if(__name__=='__main__'):
    engine=STTEngine()
    cm.console_output(engine.recognize_input())
    cm.console_output("listened")