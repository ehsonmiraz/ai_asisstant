...
class TTS:
    ...
    def _set_voice_engine():
        ...
        tts_engine.setProperty('rate', settings.SPEECH_CONFIG.get('rate'))  # Setting up new voice rate
        tts_engine.setProperty('volume',settings.SPEECH_CONFIG.get('volume')) # Setting up volume level  between 0 and 1
        tts_engine.setProperty('voice', settings.SPEECH_CONFIG.get('voice'))
        return tts_engine
...
if(__name__=='__main__'):
    TTSEngine().respond(" hello how are you babu i love you starting from the bottom now we here")